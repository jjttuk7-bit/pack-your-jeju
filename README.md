...
- **여행 동선 비교**: Day별 현재·추천 순서의 시간과 거리를 비교하고, 실제 경로와 예상 동선을 구분한 뒤 사용자가 승인할 때만 순서를 반영합니다.
...
- **내 여행 동선**: 자동차·대중교통·도보 기준의 날짜별 번호 마커와 경로선을 확인하고 현재 순서 유지 또는 추천 순서 적용
...
| `NAVER_DIRECTIONS_CLIENT_ID` | 선택. 서버 전용 네이버 Directions Client ID. 없으면 예상 동선 fallback |
| `NAVER_DIRECTIONS_CLIENT_SECRET` | 선택. 서버 전용 네이버 Directions Secret. 프런트 번들 노출 금지 |
...
| API | 역할 |
| --- | --- |
| `POST /weather/report` | 플랜의 지역·날짜·시간대와 기상청 예보를 연결해 영향과 사용자 승인형 변경안을 반환 |
| `POST /route/plan` | Day별 현재·추천 순서, 이동시간·거리·구간, 실제/예상 경로 상태와 사용자 승인형 재정렬 제안을 반환 |

`/route/plan`은 네이버 Directions 인증이 없거나 외부 호출이 실패해도 좌표가 확인된 장소를 직선거리 기반 예상 동선으로 비교합니다. 이 경우 UI는 반드시 `예상 동선`으로 표시하며 실제 도로 경로로 표현하지 않습니다.
...
- 사용자 승인형 날짜별 동선 추천·기존 순서 유지·되돌리기와 실제/예상 경로 구분
