...
하루방 패널은 통계판이 아니라 의사결정 보조 패널이다. 따라서 `확인 후보 n곳`, `신뢰 신호 n개` 같은 숫자만 보여주는 구성을 지양하고, 다음 순서로 사용자가 바로 판단할 수 있게 구성한다.

1. 하루방 판단: 현재 지역·순간·목적 조합이 여행 준비에 적합한지 한 문장으로 설명한다.
2. 좋은 점: 이미 플랜에 들어온 공공데이터 후보나 강한 순간을 알려준다.
3. 보완점: 아직 비어 있는 순간이나 확인 근거가 약한 항목을 알려준다.
4. 다음 행동: 후보 담기, 확인 근거 약한 순간 보기, 직접 추가 장소 검증 같은 버튼을 제공한다.

이 구조는 사용자가 “그래서 지금 무엇을 누르면 되는지”를 바로 알 수 있게 하기 위한 것이다.
